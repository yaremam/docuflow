...
|---|---|---|---|
| Expiry dates + renewal reminders | [031](tdr/031_expiry_dates_and_renewal_reminders_design.md) | `date_expires`/`ocr_suggested_date_expires` columns gated to confirmed `doc_type` in {insurance, contract, bill, id}; keyword-anchored extraction (`src/expiry_extract.rs`, reusing `date_extract`'s `pub(crate)` date-shape parsers) requires a nearby trigger phrase, unlike `date_extract`'s keyword-free scan; new `expiry_status` OR-multi-select facet (Expired/Expiring soon/Later/No expiry set, 14-day window); unconditional dashboard "expiring soon" strip that ignores active facets/search by design | Gating by confirmed (not suggested) `doc_type` avoids surfacing the field before classification is trusted; keyword-anchoring avoids false positives since expiry dates share the same shapes as issued dates; the strip answers "what needs attention now" independent of whatever's currently filtered, so its tests must scope assertions to their own markup, not the whole page body (TDR 031 §3-4) |
